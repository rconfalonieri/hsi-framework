...
%% load spatial classifier and classify test data
load('pretrained_models/spatial_multiblock_32x32_nopadding.mat');
[YTest, score] = classify(multiblock_classifier_nopadding_32, imagesTest);
...
wrong_classified = find((YTest(:) == imagesLabelsTest(:))==0);


%% try (hopefully) final version of reshaping + move_normalization
load('pretrained_models/spatial_multiblock_32x32_nopadding.mat');
...
accuracy3d_final = sum(YTest_3d_final(:) == imagesLabelsTest(:))/numel(imagesLabelsTest(:));
confmat3d_final = confusionmat(imagesLabelsTest(:),YTest_3d_final(:), 'Order', {'clearwood', 'softrot', 'brownstain', 'bluestain'});
